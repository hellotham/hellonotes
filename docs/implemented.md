...
- **Verified on the simulator** (`iOSEditorSnapshotTests`): live inline styling (bold, italic, inline-code with background, strikethrough, coloured links), caret-driven **concealment** of markers, heading sizes, dimmed blockquotes, coloured callout title/body, and coloured ordered-list numbers all render correctly on iOS.
- **Known iOS gap (documented in [unimplemented.md](unimplemented.md)):** the custom-fragment *chrome* (unordered-list bullet glyphs, callout tint band/bar/icon, blockquote gutter bar, task checkboxes, heading bottom-rule) doesn't paint on `UITextView` — unlike `NSTextView`, `UITextView` doesn't invoke a custom `NSTextLayoutFragment.draw`. Needs an overlay-view renderer on iOS. Inline styling + concealment (the core) are unaffected.
