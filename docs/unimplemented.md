...
- 🍎 **macOS-only surfaces** — Open Quickly, the Git UI, Mermaid preview, HTML/PDF export, open-in-new-window, Graph/Mind Map, and the file viewer. *(Off this list since 1.3.2: image paste, multi-tab, Marp slides, Find & Replace, `[[wiki-link]]` / `#tag` autocomplete, inline completion and heading navigation — see implemented.md §24.)* *(The AI stack is no longer on this list: 1.3 brought Summarise / Suggest Tags / Suggest Links / Rewrite, the selection actions, Ask Library, the Assistant, Review Links and New Note from a Prompt to iOS — see implemented.md.)*
