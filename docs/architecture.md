...
│    sidebar (collections + folders) · pane · inspector, by      │
...
The guiding principle from the project rules: **native Apple frameworks first, no WebViews in the editing path, async/await.** `WKWebView` appears only in read-only rendering surfaces — the GFM **Preview** (`MarkdownEditor/GFMPreview.swift`, which deliberately renders through cmark-gfm for GitHub-identical output on both macOS and iOS) and the **Marp slides** preview (`UI/SlidesView.swift`) — never in the live editor. Below, each capability is evaluated against alternatives.
...
> shipped editor is `Packages/NotesEditor` — see [implemented.md](implemented.md).
...
### 5.4 Code-block syntax highlighting  ⟶ **HighlighterSwift** ✅ (installed, direct)
| Option | Notes | Verdict |
|---|---|---|
| **HighlighterSwift** (highlight.js core, native rendering) | Used directly via the editor's `CodeHighlighting` protocol (`CodeHighlighterAdapter`); GitHub light/dark theme (matches the Preview); ~190 languages. | **Adopted** — editor takes foreground colours only, cached per content hash. |
...
### 5.5 Math rendering  ⟶ **SwiftMath** ✅ (installed, direct)
| Option | Notes | Verdict |
|---|---|---|
| **SwiftMath** | Native LaTeX math typesetting (no WebView); used directly by `UI/MathImageRenderer.swift`, which renders `$…$`/`$$…$$` to images the editor draws in-line. | **Adopted** — native, no WebView. |
...
| 4 Shell | `UI/Shell/LibraryPlace.swift` | The library-wide places pinned above the collections — Recents and Bookmarks, each spanning every open collection (`LibraryRail.swift` was deleted with the rail) |
...
| 4 Shell | `MacContentView.swift` | macOS slots for `AdaptiveShell` — the sidebar's collection tree, editor column, inspector |
...
| Risk | Mitigation |
|---|---|
| Editor correctness at scale (custom TextKit 2) | The `Packages/NotesEditor` package is unit + perf + GFM-conformance tested (83 tests, incl. 648-case GFM spec parity); `OSSignposter`-gated perf budgets fail CI on regression. |
