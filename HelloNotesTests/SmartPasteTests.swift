...
struct SmartPasteTests {
    ...
    func bareURLBecomesAMarkdownLink() {
        let result = SmartPaste.urlLink(fromString: SmartPaste.pasteboardString(pasteboard(string: "https://example.com/page")))
    ...
    func nonURLTextIsNotTreatedAsALink() {
        #expect(SmartPaste.urlLink(fromString: SmartPaste.pasteboardString(pasteboard(string: "just some text"))) == nil)
        #expect(SmartPaste.urlLink(fromString: SmartPaste.pasteboardString(pasteboard(string: "see https://x.com now"))) == nil) // not a bare URL
        #expect(SmartPaste.urlLink(fromString: SmartPaste.pasteboardString(pasteboard(string: "/local/path"))) == nil)
    }
    ...
    func plainHTMLWithoutFormattingFallsThrough() {
        ...
        #expect(SmartPaste.markdownFromHTML(html: SmartPaste.pasteboardHTML(pasteboard(html: "<p>just a paragraph</p>"))) == nil)
    }

    @Test
    func convertsBoldItalicAndLinks() throws {
        let html = "<p>This is <b>bold</b> and <i>italic</i> and a <a href=\"https://x.com\">link</a>.</p>"
        let md = try #require(SmartPaste.markdownFromHTML(html: SmartPaste.pasteboardHTML(pasteboard(html: html))))
